...
#include "pubsub.h"                 // Board Support Package
...
void BSP_poolBtn(void)
{
   ...
   {
      BSP_sendUart("key 1 \r\n");
      QEvt *pe = Q_NEW(QEvt, BTN_SW1_SIG);
      QACTIVE_POST(&Blinky_inst.super , pe, 0U);
   }
   if(key == '2')
   {
      BSP_sendUart("key 2 \r\n");
      QEvt *pe = Q_NEW(QEvt, BTN_SW2_SIG);
      QACTIVE_POST(&Blinky_inst.super , pe, 0U);
   }
   if(BSP_btn1())
   {
...
bool BSP_btn1(void)
{
    return !GPIOF_AHB->DATA_Bits[BTN_SW1];
}
bool BSP_btn2(void)
{
    return !GPIOF_AHB->DATA_Bits[BTN_SW2];
}
